    ...
    // alert(spaObject['url'])
      if (response.login){
        ...
        if (window.history.replaceState) {
          ...
          window.history.replaceState(window.location.hash, document.title,  '#/login?next=' + nextUrl);
       }
        ...
        if (response.redirecturl) {
          spaObject[response.redirecturl] = {}
          spaObject[response.redirecturl]['url'] = response.redirecturl
          spaObject[response.redirecturl]['reload'] = true
          Main.get('#' + spaObject[response.redirecturl]['url'], function (context) {
            loadSpaRedirect(spaObject[response.redirecturl])
          })
          Main.routes.get.unshift(Main.routes.get.pop());
...
function reloadApp(spaObject, response) {
  ...
  if (response.login){
    ...
    if (window.history.replaceState) {
      //prevents browser from storing history with each change:
      window.history.replaceState(window.location.hash, document.title,  '#/login?next=' + spaObject['url']);
   }
    nextUrl =spaObject['splat']
    // window.location = '#/login?next=' + spaObject['url']
    Main.swap('');
        document.title = response.title
    Main.$element().append(response.content)
