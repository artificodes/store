  ...
  const toggleLGA = target => {
    let state = target.value,                                                         // Get value of state
      ...
      lgaSelect = $(".select-lga")[0],                                  // Get the LGA select element
      length = lgaSelect.length;                                              // Get number of options already existing in LGA select element

    // Clear LGS select element
    for (i = length - 1; i >= 0; i--) {
      lgaSelect.length[i] = null;
    }
