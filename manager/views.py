...
from manager.email_sender import sendmail
...
from manager import forms as aforms
...
# from manager import forms as pforms
...
def manager_login_required(function):
    ...
    def checkstatus(request,*args,**kwargs):
        if request.user.is_anonymous or not request.user.is_superuser:
            return managerlookup(request,True,*args,**kwargs)
        else:
            return function(request,*args,**kwargs)

    return checkstatus

# def checkmanagertatus(function):
    
#     def managerstatus(request,*args,**kwargs):
#         allObject = inherit(request)

#         try:

#             store = mmodels.Store.objects.get(user=request.user)
...
def inherit(request):
    ...
    if request.user.is_anonymous:
        ...
        try:
            ...
            store = mmodels.Store.objects.all()[0]
            # socialplatforms = mmodels.SocialLink.objects.filter(store=store)
...
def confirmemail(request,updated=False, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/email_confirmation_form.html'
...
def storesetup(request,inline=False, *args, **kwargs):
    ...
    if request.method == 'POST':
        try:
            store = mmodels.Store.objects.get(user=request.user)
            store = aforms.InitialSetup(request.POST,request.FILES,instance=store)

        except ObjectDoesNotExist:

            store = aforms.InitialSetup(request.POST,request.FILES)
        if store.is_valid:
            ...
            'redirecturl':redirect('manager_dashboard').url
        ...
        template_name = 'manager/setup.html'
        ...
        if inline:
            ...
            'content': content,
            'title':"Store setup",
            'redirecturl':redirect('manager_store_setup').url
...
def editstorebranding(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        template_name = 'manager/edit_store_branding.html'
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
...
def editstorepayment(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        template_name = 'manager/edit_store_payment.html'
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
        'modal_content': content,
...
def editstoreinformation(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        template_name = 'manager/edit_store_information.html'
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
        'modal_content': content,
...
def editproductinformation(request,productid=None, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if product.is_valid:
            ...
            output_data = {
            'notification': message,
            'redirecturl': redirect('manager_product_details',productid=product.productid).url,
            'container':'app'
        ...
        template_name = 'manager/edit_product_information.html'
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
        'content': content,
        'title': title,
...
def addproductvariation(request,productid=None, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        template_name = 'manager/add_product_variation.html'
        allObject['next'] = request.GET.get('next')
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
        'modal_content': content,
...
def addsocialink(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        template_name = 'manager/add_social_link.html'
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
        'modal_content': content,
        'heading': title
...
def addmeasurementunit(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        template_name = 'manager/add_measurement_unit.html'
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
        'modal_content': content,
        'heading': title
...
def addproductimage(request,productid=None, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        allObject = inherit(request)
        url = str(request.get_full_path())
        template_name = 'manager/add_product_image.html'
        allObject['product'] = product
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
        'modal_content': content,
...
def select_account_type(request,*args, **kwargs):
    ...
    managertatus = checkmanagertatus(request, customer)
    ...
    template_name = 'manager/select_account_type.html'
    return render(request,template_name,allObject)
...
def cart(request, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/cart.html'
    # customer = allObject['customer']
...
def overview(request, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/overview.html'
...
def productmonthlysalesgraph(request,productid=None, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/monthly_sales_graph.html'
    product = apmodels.Product.objects.get(productid=productid)
...
def product_overview(request,productid=None, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/product_overview.html'
    product = apmodels.Product.objects.get(productid=productid)
    revenue = 0
...
def product_information(request,productid=None, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/product_information.html'
    product = apmodels.Product.objects.get(productid=productid)
...
def product_variations(request,productid=None,inline=False, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/product_variations.html'
    product = apmodels.Product.objects.get(productid=productid)
...
def orderspage(request, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/orders_page.html'
...
def productspage(request, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/products_page.html'
    content = loader.render_to_string(template_name,allObject,request)
...
def search_products(request, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/search_result.html'
    if request.method == 'POST':
...
def remove_order(request,id=None, *args, **kwargs):
    ...
    message = render_to_string('manager/order_cancelled.html', {
            'order':order,
    ...
    allObject['orders'] = orders
    content_template = 'manager/order_container.html'
    content = loader.render_to_string(content_template,allObject,request)
...
def myorders(request, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/my_orders.html'
    # customer = allObject['customer']
...
def allproductspage(request, *args, **kwargs):
    ...
    template_name = 'manager/all_products.html'
    # customer = allObject['customer']
...
def categorys(request,name=None, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/category_page.html'
    # customer = allObject['customer']
...
def slideshowpage(request,slug=None, *args, **kwargs):
    ...
    template_name = 'manager/slideshow_page.html'
    # customer = allObject['customer']
...
def slideshowproducts(request,slug=None, *args, **kwargs):
    ...
    #     product.save()
    allObject['products'] = products
    content_template = 'manager/products.html'
    content = loader.render_to_string(content_template,allObject,request)
    output_data = {
    'content': content,
                    }
...
def brandpage(request,slug=None, *args, **kwargs):
    ...
    allObject = inherit(request)
    template_name = 'manager/brand_page.html'
    # customer = allObject['customer']
...
def flashsalespage(request,id=None, *args, **kwargs):
    ...
    allObject = inherit(request)
    template_name = 'manager/flash_sales_page.html'
    # customer = allObject['customer']
...
def homegroup(request,slug=None, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/home_group_page.html'
    # customer = allObject['customer']
...
def saveditemspage(request,slug=None, *args, **kwargs):
    ...
    allObject = inherit(request)
    template_name = 'manager/saved_items_page.html'
    # customer = allObject['customer']
...
def product_details(request,productid=None,inline=False, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/product_details.html'
    product = apmodels.Product.objects.get(productid=productid)
    product.save()
    allObject['year']=now().year
...
def store_information(request,inline=False, *args, **kwargs):
    ...
    template_name = 'manager/store_information.html'
    store = mmodels.Store.objects.get(user=request.user)
...
def store_payment(request,inline=False, *args, **kwargs):
    ...
    template_name = 'manager/store_payment.html'
    store = mmodels.Store.objects.get(user=request.user)
...
def store_branding(request,inline=False, *args, **kwargs):
    ...
    template_name = 'manager/store_branding.html'
    store = mmodels.Store.objects.get(user=request.user)
...
def store_socials(request,inline=False, *args, **kwargs):
    ...
    template_name = 'manager/store_social_media_platforms.html'
    socials = gmodels.SocialLink.objects.all()
...
def store_measurement_units(request,inline=False, *args, **kwargs):
    ...
    template_name = 'manager/store_measurement_units.html'
    units = apmodels.MeasurementUnit.objects.all()
...
def allhomegroupsection(request,*args, **kwargs):
    ...
    content_template = 'manager/home_group_container.html'
...
def newproducts(request,*args, **kwargs):
    ...
    content_template = 'manager/new_products.html'
    content = loader.render_to_string(content_template,allObject,request)
    output_data = {
    'content': content,
                    }
...
def flashsaleproduct(request,id=None, *args, **kwargs):
    ...
    allObject['productimages'] = productimages
    allObject['product'] = product
    content_template = 'manager/flash_sale_product.html'
    content = loader.render_to_string(content_template,allObject,request)
    output_data = {
    'content': content,
                    }
...
#         pass
#     allObject['product'] = product
#     content_template = 'manager/product.html'
#     content = loader.render_to_string(content_template,allObject,request)
#     output_data = {
#     'content': content,
#                     }
...
def cartedproduct(request,id=None, *args, **kwargs):
    ...
    content_template = 'manager/carted_product.html'
    content = loader.render_to_string(content_template,allObject,request)
    output_data = {
    'content': content,
                    }
...
def slideshows(request,*args, **kwargs):
    ...
    content_template = 'manager/slideshows.html'
    content = loader.render_to_string(content_template,allObject,request)
    output_data = {
    'content': content,
                    }
...
def paginatedproducts(request,productid=None, *args, **kwargs):
    ...
    content_template = 'manager/paginated_products.html'
    content = loader.render_to_string(content_template,allObject,request)
    output_data = {
    'content': content,
                    }
    ...
    if request.method == 'POST':
        ...
        if delivery != 'p':
            if customer.contact_ready:
                ...
                content_template = 'manager/contact_form.html'
                content = loader.render_to_string(content_template,allObject,request)
                url = str(request.get_full_path())
        ...
        allObject['order'] = order
        content_template = 'manager/order_placed.html'
        content = loader.render_to_string(content_template,allObject,request)
        output_data = {
        'modal_message': content,
                        }
                ...
                for flashsale in flashsales:
                    for flashsaleproduct in flashsale.products.all():
                        if product == flashsaleproduct.product:
                            allObject['flashsaleproduct'] = flashsaleproduct
                            break
                content_template = 'manager/cart_form_modal.html'
                content = loader.render_to_string(content_template,allObject,request)
                output_data = {
                'modal_message': content,
                                }
...
def add_to_saved(request,productid=None, *args, **kwargs):
    ...
    if product in saved.products.all():
        ...
        allObject['product'] = product
        add_to_saved_button = 'manager/remove_from_saved_button.html'
...
def remove_from_saved(request,productid=None, *args, **kwargs):
    ...
    if product in saved.products.all():
        ...
        allObject['product'] = product
        add_to_saved_button = 'manager/add_to_saved_button.html'
        content = loader.render_to_string(add_to_saved_button,allObject,request)
        output_data = {
            'content':content,
...
def checkout(request,productid=None, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        template_name = 'manager/checkout.html'
        # customer = allObject['customer']
...
def remove_from_cart(request,id=None, *args, **kwargs):
    ...
    allObject['cart'] = cart
    content_template = 'manager/cart_container.html'
    content = loader.render_to_string(content_template,allObject,request)
...
def index(request, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/base/base.html'
    store = allObject['store']
    try:
...
def app(request, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/base/app.html'
    store = allObject['store']
    try:
        allObject['title']=store.store_name
    except Exception:
...
@method_decorator(manager_login_required, name='get')
# @method_decorator(checkmanagertatus, name='get')
...
def downloadorder(request,orderid=None,*args, **kwargs):
    ...
    template_name = 'manager/order_download.html'
    response = PDFTemplateResponse(request=request,
                                    template='manager/order_download.html',
...
def myorder(request,orderid=None, *args, **kwargs):
    allObject = inherit(request)
    template_name = 'manager/my_order.html'
    # customer = allObject['customer']
...
def orderpaid(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        message = render_to_string('manager/order_placed_email.html', {
                'order':order,
                'customer':customer,
            'title':'ORDER PLACED',
            'socials':gmodels.SocialLink.objects.all(),
        ...
        message = render_to_string('manager/customer_order_placed_email.html', {
                'order':order,
                'customer':customer,
                'title':'ORDER PLACED',
                'socials':gmodels.SocialLink.objects.all(),
...
def investmentbrief(request, *args, **kwargs):
    ...
    template_name = 'manager/investment_brief.html'
    content = loader.render_to_string(template_name,allObject,request)
    output_data = {
    'details':content,
                    }
...
def generatepinqrcode(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        template_name = 'manager/qrcode_response.html'
...
def creditmodalcontent(request, *args, **kwargs):
    ...
    template_name = 'manager/credit_modal_content.html'
    content = loader.render_to_string(template_name,allObject,request)
    output_data = {
                        'content':content,
                    }
...
def share(request, *args, **kwargs):
    ...
    template_name = 'manager/share_form.html'
    content = loader.render_to_string(template_name,allObject,request)
    output_data = {
                        'content':content,
                        'title':'Fetch Share'
...
def withdraw(request, *args, **kwargs):
    ...
    if request.method =='POST':
        ...
        try:
            ...
            template_name = 'manager/withdrawal_form.html'
        except ObjectDoesNotExist:
            bank = apmodels.Bank.objects.get(name='Gtbank')
            allObject['bank']=bank
            template_name = 'manager/no_account.html'
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
                            'content':content,
...
def deposit(request, *args, **kwargs):
    ...
    if request.method =='POST':
        ...
        template_name = 'manager/deposit_form.html'
...
def invest(request, *args, **kwargs):
    ...
    if request.method =='POST':
        ...
        template_name = 'manager/invest_form.html'
...
def addaccount(request, *args, **kwargs):
    ...
    if request.method =='POST':
        ...
        template_name = 'manager/add_account_form.html'
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
                            'content':content,
...
def choosetransferdestination(request,destination=None, *args, **kwargs):
    allObject = inherit(request)
    customer = allObject['customer']
    template_name = 'manager/choose_transfer_destination.html'
    content = loader.render_to_string(template_name,allObject,request)
    output_data = {
                        'content':content,
...
def internaltransfer(request, *args, **kwargs):
    ...
    if request.method =='POST':
        ...
        banks = apmodels.Bank.objects.all()
        allObject['banks']= banks
        template_name = 'manager/internal_transfer_form.html'
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
                            'content':content,
...
def managerlookup(request,inline=False, *args, **kwargs):
    allObject = inherit(request)
    if request.user.is_authenticated:
        auth.logout(request)
    # if request.user.is_authenticated:
    ...
    if request.method == 'POST':
        ...
        try:
            user = User.objects.get(email=uniqueidentifier,is_superuser=True)
        except ObjectDoesNotExist:
            try:
                user = User.objects.get(username=uniqueidentifier,is_superuser=True)
            except ObjectDoesNotExist:
        ...
        template_name = 'allauth/account/manager_enter_password.html'
        content = loader.render_to_string(template_name,allObject,request)
        output_data = {
        ...
        template_name = 'allauth/account/manager_lookup.html'
        nexturl = redirect('manager_dashboard').url
...
def logoutuser(request, *args, **kwargs):
    ...
    output_data = {
                        ...
                        'title':'Login',
                        'login':True,
                        'nexturl':request.GET.get('next') or redirect('manager_dashboard').url
                    }
    return JsonResponse(output_data)
...
def managersignup(request, *args, **kwargs):
    allObject = inherit(request)
    if request.method == 'POST':
        chars = '0123456789' 
        token = ''
        ...
        if request.GET.get('next'):
            allObject['next_page']=request.GET.get('next')
            allObject['redirect_url']=''.join(tuple(request.GET.get('next')))
        template_name = 'allauth/account/manager_signup.html'
        nexturl = redirect('manager_dashboard').url
