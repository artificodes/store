    ...
    if(app){
      ...
      $.ajax({
        ...
        success: function (response) {
          // alert(spaObject['splat'])
          // reloadApp(spaObject,response)
...
var Main = $.sammy(function () {
  ...
  this.post('#/manager/new/accounts/signup/', function (context) {
  ...
  this.get(/\#\/(.*)/, function (context) {
    if(this.params['splat'].join('/').startsWith('/')){
    ...
    // alert(spaObject['url'])
      if (response.login || response.signup){
        ...
        if (window.history.replaceState) {
          window.history.replaceState(window.location.hash, document.title,  '#/manager/login?next=' + nextUrl);
       }
        ...
        if (window.history.replaceState) {
          window.history.replaceState(window.location.hash, document.title,  '#/manager/signup?next=' + nextUrl);
       }
      }
      Main.swap('');
            document.title = response.title
